...
sdrandom <- sd3[sample(nrow(sd3), nrow(sd3)),] #Get a random sample since the data is organized by participant

sdclean <- na.omit(sdrandom) #Remove rows with NA values to create a "clean" set

names(sdclean)[69] <- "wave"
sdclean[, 69] <- as.factor(sdclean[, 69])

nrow(sdclean)

sdclean$wave<- factor(sdclean$wave, levels = c(5, 5, 6, 6, 6, 2, 2, 3, 3, 4, 4, 5, 5, 5, 1, 1, 1, 3, 3, 3, 3), labels = c("October", "October", "November", "November", "November", "March", "March", "April", "April", "September", "September", "October", "October", "October", "February", "February", "February", "April", "April", "April", "April"))
...
```{r}
#Subsetting male
Mexplore <- subset(sdclean, sdclean$gender == 1 )
...
#Female matching
```{r}
Fexplore <- subset(sdclean, sdclean$gender == 0 )
